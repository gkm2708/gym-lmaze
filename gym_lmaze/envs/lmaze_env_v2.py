...
class LmazeEnv_v2(gym.Env):
  ...
  def reset(self):                        # just reset
    ...
    self.image_wall_previous = np.asarray(self.state[0, self.ball_x0 - int(self.fovea/2) : self.ball_x0 + int(self.fovea/2+1),
                                            self.ball_y0 - int(self.fovea/2) : self.ball_y0 + int(self.fovea/2+1)])
    self.image_wall_current = np.asarray(self.state[0, self.ball_x0 - int(self.fovea/2) : self.ball_x0 + int(self.fovea/2+1),
                                            self.ball_y0 - int(self.fovea/2) : self.ball_y0 + int(self.fovea/2+1)])
    self.image_goal_previous[2, 2] =  1.0
    self.image_goal_current[2, 2] = 1.0
    self.image_global_goal = np.asarray(self.state[1, self.ball_x0 - int(self.fovea/2) : self.ball_x0 + int(self.fovea/2+1),
                                        self.ball_y0 - int(self.fovea/2) : self.ball_y0 + int(self.fovea/2+1)])
  ...
  def step(self, goal):
      ...
      self.f_goal_x = self.ball_x0 + int(goal / self.fovea) - int(self.fovea/2)
      self.f_goal_y = self.ball_y0 + int(goal % self.fovea) - int(self.fovea/2)
      ...
      if self.VISUALIZE:
          ...
          '''
          imagel1 = np.concatenate([self.boundary,
                                    ...
                                    self.boundary,
                                    self.boundary,
                                    self.boundary], axis = 1)
          imagel2 = np.concatenate([self.boundary,
                                    ...
                                    self.boundary,
                                    self.boundary], axis = 1)
          imagel3 = np.concatenate([self.boundary,
                                    image_wall_current_,
                                    self.boundary,
                                    self.boundary,
                                    self.boundary,
                                    ...
                                    self.boundary,
                                    self.boundary ], axis = 1)
          imagel4 = np.concatenate([self.boundary,
                                    image_goal_current_,
                                    self.boundary,
                                    self.boundary,
                                    self.boundary,
                                    ...
                                    self.boundary,
                                    self.boundary], axis = 1)
          imagel5 = np.concatenate([self.boundary,
                                    image_global_goal_,
                                    self.boundary,
                                    self.boundary,
                                    self.boundary,
